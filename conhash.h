...
/* nodes structure */
struct node_s       //真实结点的数据结构
{
	char iden[64]; /* node name or some thing identifies the node */    //结点的名称或ＩＤ，用来标识结点
    u_int replicas; /* number of replica virtual nodes */   //虚拟结点的个数
    u_int flag;     //结点的状态，只有两种状态，一种是初始化状态，即没有添加到服务器中，另一种状态表示添加到了服务器中
};
